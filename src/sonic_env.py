# Based on Thomas Simonini tutorial and adborghi's implementation
...
# useful for stacking frames
from baselines.commmon.atari_wrappers import FrameStack
...
class PreprocessFrame(gym.ObservationWrapper):
    ...
    def __init__(self, env):
        gym.ObservationWrapper.__init_(self, env)
        self.width = 96
        self.height = 96
...
class ActionsDiscretizer(gym.ActionWrapper):
    ...
    def __init__(self, env):
        ...
        For each action in actions
            ...
            For each button in action: (for instance['LEFT']) we need to make that left button
                - Then the button index = LEFT = True

            In fact at the end we will have an array where each is an action and each elements
...
class AllowBacktracking(gym.Wrapper):
    ...
    def step(self, action): # pylint: disable=E0202
        ...
        self._cur_x += rew
        rew = max(0, self._max_x - self._max_x)
...
def make_env(env_idx):
    ...
    # Allow back tracking that helps agents, so they are not discouraged too heavily
    # from exploring backwards if there is no way to advance head-on in the level
    env = AllowBacktracking(env)

    return env

# make train env for SpringYardZone.Act3
def make_train_0():
    return make_env(0)
