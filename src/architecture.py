...
class A2CPolicy(object):
    def __init__(self, sess, ob_space, action_space, nbatch, nsteps, reuse = False):
        ...
        with tf.variable_scope("model", reuse = reuse):
            ...
            # 2 fc streams: one for the actor and one for critic
        ...
        # needed to avoid error in code
        self.initial_state = None

        # Take an action in the action distribution (remember we are in a situation of stochastic
        # policy so we don't always take the action with the highest probability for instance if we
        # have 2 actions 0.7 and 0.3 we have 30% chance to take the second)
        a0 = self.pd.samples()
